...
void DrawAllPoints(Image *img, int points[], int pointsSize){
	...
	if (pointsSize%2!=0) {
		//On donne un tableau de int si impaire alors tableau avec un abscisse seul
		printf("Mauvaise longueur de point donné");
		return;
	}

	//Saute de 2 pour avoir x et y
		...
		if(!equalColor(img->_buffer[i][p.y],C_new(0.f,0.f,0.f))){
			int haveSommetInLine = 0;
				...
				if(sommet != NULL){
					...
					//Pour eviter la succesion de point blanc
...
int isSameAbscisseAsSommet(ListePoints* list,int x){
	...
	//Parcours toute la liste chainé
	while(tmp != NULL){
		if(tmp->point.x == x)
			return 1;
		tmp = tmp->next;
...
// 3.3 1) Remplie l'image avec la méthode de FAIN
void fill(Image *img,ListePoints* list){
	Point boundingBox[2];
	FindBoundingBox(list,boundingBox);
	int Xintersect[100];
		...
		for(int i = boundingBox[0].x; i <= boundingBox[1].x ; i++){
			if(!equalColor(img->_buffer[i][j],C_new(0.f,0.f,0.f))){
				if(!equalColor(img->_buffer[i+1][j],C_new(0.f,0.f,0.f))){
					//Pour eviter les points blancs avant un sommet
...
//Remplie le polygone avec la méthode scan-line de LMG
...
//Free list
//Utilisation d'un double pointeur pour la free
...
//------------------ SELECTION CLAVIER ------------------
//Taille d'un coté du carre de selection
...
void selectSommet(Image* img,int x, int y, Color* save){
	...
	for(int j=x-Size_Select; j<x+Size_Select;j++){
		for(int i=y-Size_Select; i<y+Size_Select;i++){
			//Sauvgarde ce qu'il y avait avant pour ne redessiner que le carré de selection
...
//Efface les lignes lié au point en redessiner bresenham mais noir
...
//EDGE
//Dessine l'edge en couleur rouge
...
void closestEdge(ListePoints* list, Points** edgePoint, int x, int y){
	...
	if(distPrev<distNext){
		if(closestPoint->previous == NULL){//si tete
		...
		if(closestPoint->next == NULL){//si queue
			edgePoint[0]  = closestPoint->previous;
			edgePoint[1] 	= closestPoint;
		}else{
			edgePoint[0]  = closestPoint;
...
//Evite d'avoir un point nul dans la selection aprés suppression
int isOnEdgeSelect(Points* cible, Points** edgePoint){
	if(cible != NULL || edgePoint != NULL){

		if(cible->point.x == edgePoint[0]->point.x && cible->point.y == edgePoint[0]->point.y){
			return 1;
		}

		if(cible->point.x == edgePoint[1]->point.x && cible->point.y == edgePoint[1]->point.y){
			return 1;
		}
